...
def evaluate_schedule_soft(schedule: Schedule) -> float:
    """Evaluate soft constraints only — skips the expensive check_all_hard().

    Use this for candidates already validated as hard-feasible
    (e.g. from is_valid_assignment() or _random_same_day_swap()).
    This is ~3-4x faster than evaluate_schedule().
    """
    score = 0.0
